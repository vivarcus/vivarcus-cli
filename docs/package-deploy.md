...
配合 [vivarcus-sdk](https://github.com/vivarcus/vivarcus-sdk) 构建的 `gosdk/` VPK，用 `ov` 导入、校验并部署。

## 前置

- 已安装与 Vault 版本对齐的 `ov`（见仓库 [README](../README.md)）
- 已登录或配置 `OV_TOKEN` / `OV_ENDPOINT` / `OV_VAULT`
...
```bash
# 1. 导入
ov package import ./my-action.vpk --json
# 记录返回的 package_id
...
| `ov component apply-mdl` | metadata 编辑权限 |
